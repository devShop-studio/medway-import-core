 ...
 * Signed: EyosiyasJ
 */
export function decomposeConcatenatedCell(raw, opts) {
    var _a, _b, _c;
    const cleaned = String(raw !== null && raw !== void 0 ? raw : "").trim();
    if (!cleaned)
    ...
    const hasExtraction = (field) => extractions.some((e) => e.field === field);
    ...
    if (fallbackApplied) {
        const candidate = (_a = splitFallback.generic_name) !== null && _a !== void 0 ? _a : splitFallback.leftover;
        if (candidate)
            leftover = candidate;
    }
    ...
    if (((_b = opts === null || opts === void 0 ? void 0 : opts.mode) !== null && _b !== void 0 ? _b : "default") === "opportunistic") {
        const accepted = opportunisticAccept(cleaned, extractions, leftover, (_c = opts === null || opts === void 0 ? void 0 : opts.minSignals) !== null && _c !== void 0 ? _c : 3);
        if (!accepted)
            return { leftover: "", extractions: [] };
    }
    return { leftover, extractions };
 ...
 * Signed: EyosiyasJ
 */
export function splitNameGenericStrengthForm(raw) {
    const result = {};
    if (!raw)
        return result;
    ...
    if (hyMatch) {
        ...
        if (canonical) {
            form = canonical;
            beforeForm = s.slice(0, hyMatch[1].length).trim();
        }
    }
    ...
    let last = null;
    let m;
    while ((m = re.exec(beforeForm)) !== null)
        last = m;
    if (last) {
    ...
    if (!form && typeof leftover === "string" && leftover) {
        ...
        if (cand) {
            result.form = FORM_SYNONYMS[cand];
            leftover = undefined;
        }
    }
    if (leftover)
    ...
    for (const [canonical, variants] of Object.entries(FORM_DICTIONARY)) {
        ...
        for (const v of sorted) {
            ...
            if (lower.endsWith(vv)) {
                if (canonical === "other" && !hasDigitOrUnit)
                    continue;
...
function detectStrength(tokens, out) {
...
function hasDigit(s) { return HAS_DIGIT_RE.test(String(s !== null && s !== void 0 ? s : "")); }
function hasUnitToken(s) { return UNIT_RE.test(String(s !== null && s !== void 0 ? s : "")); }
...
function isAllowedNumericBrandCell(s) { return HAS_DIGIT_RE.test(String(s !== null && s !== void 0 ? s : "")) && ALLOWED_NUMERIC_BRANDS.has(String(s !== null && s !== void 0 ? s : "").trim().toLowerCase()); }
function isAllowedNumericManufCell(s) { return HAS_DIGIT_RE.test(String(s !== null && s !== void 0 ? s : "")) && ALLOWED_NUMERIC_MANUF.has(String(s !== null && s !== void 0 ? s : "").trim().toLowerCase()); }
function isAlphaNumericBatch(s) {
    const v = String(s !== null && s !== void 0 ? s : "");
    return ALNUM_BATCH_RE.test(v) && /[A-Za-z]/.test(v);
}
function detectForm(tokens, out) {
    for (const tok of tokens) {
        if (tok.consumed)
            continue;
...
function detectPackContents(tokens, out) {
    for (const tok of tokens) {
        if (tok.consumed)
            continue;
...
function detectCountry(tokens, out) {
    const maxLen = 3;
    for (let len = 1; len <= maxLen; len++) {
        for (let i = 0; i <= tokens.length - len; i++) {
            const slice = tokens.slice(i, i + len);
            if (slice.every((t) => t.consumed))
                continue;
...
function detectGtin(tokens, out) {
    for (const tok of tokens) {
        if (tok.consumed)
            continue;
...
function detectBatch(tokens, out) {
    for (const tok of tokens) {
        if (tok.consumed)
            continue;
...
function detectExpiry(tokens, out) {
    const isDateToken = (text) => DATE_PATTERN.test(text.trim());
...
function detectManufacturerHint(raw, out) {
...
function detectBrandHead(raw, out) {
    var _a, _b;
    ...
    const head = (_b = (_a = s.split(/[;|,]+|\s+-\s+/)[0]) === null || _a === void 0 ? void 0 : _a.trim()) !== null && _b !== void 0 ? _b : "";
    if (!head)
        return;
    if (head.length > 40)
        return;
    ...
    candidate = parts[0];
    if ((hasDigit(candidate) && !isAllowedNumericBrandCell(candidate)) || hasUnitToken(candidate))
        return;
...
function detectManufacturer(raw, out) {
 ...
 * Signed: EyosiyasJ
 */
function opportunisticAccept(raw, ex, leftover, minSignals) {
    ...
    if (!hasStrength)
        return false;
    if (additionalSignals < Math.max(0, (minSignals - 1)))
        return false;
    if (looksFormulaLike(raw) || looksFormulaLike(leftover))
        return false;
    return true;
}
