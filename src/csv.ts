 ...
 * Signed: EyosiyasJ
 */
export function parseDsvRaw(text: string, delim: "," | ";" | "\t" | "|"): string[][] {
  const rows: string[][] = [];
  let current: string[] = [];
  let field = "";
  let inQuotes = false;
...
const looksDateLike = (s: string): boolean => {
  const t = s.trim();
  return /^\d{4}-\d{2}-\d{2}$/.test(t) || /^\d{2}[\/-]\d{2}[\/-]\d{2,4}$/.test(t) || /^\d{3,5}$/.test(t);
};
const looksNumeric = (s: string): boolean => /^[-+]?\d+(?:[.,]\d+)?$/.test(s.trim());
const looksHeaderToken = (s: string): boolean => {
 ...
 * Signed: EyosiyasJ
 */
export function detectHeaderMode(rows: string[][]): "headers" | "none" | "untrusted" {
  const first = rows[0] || [];
  const second = rows[1] || [];
  if (!first.length) return "none";

  const isShortLabel = (s: string): boolean => {
  ...
  const isCountry = (s: string): boolean => /\b(ethiopia|india|germany|china|united states|united kingdom|france|italy|spain|kenya|south africa)\b/i.test(String(s).trim());
  ...
  if (headerScore >= 2 && headerScore >= dataScore) return "headers";
  if (dataScore >= 2 && dataScore > headerScore) return "none";

  // Ambiguous: compare type likeness between first and second rows
  const classify = (s: string) => {
    const t = s.trim();
    if (!t) return "empty";
    if (looksDateLike(t)) return "date";
    if (looksNumeric(t)) return "num";
    if (/\b(mg|mcg|ml|%|g|iu)\b/i.test(t) || /\//.test(t)) return "strength";
    if (/^(tablet|tab|capsule|cap|syrup|cream|ointment|inj|injection|solution)$/i.test(t)) return "form";
    return "text";
  };
  const sameTypeCount = Math.min(first.length, second.length)
    ? first.reduce((acc, v, i) => acc + (classify(String(v)) === classify(String(second[i] ?? "")) ? 1 : 0), 0)
    : 0;
  const threshSame = Math.floor(first.length * 0.6);
  if (sameTypeCount >= threshSame) return "none";
 ...
 * Signed: EyosiyasJ
 */
export function buildRawRows(rows: string[][], mode: "headers" | "none"): RawRow[] {
  const out: RawRow[] = [];
  if (mode === "headers") {
    const headers = rows[0]?.map((h) => String(h ?? "").trim()) ?? [];
    for (let r = 1; r < rows.length; r++) {
      const rowVals = rows[r];
      const obj: RawRow = {};
      headers.forEach((h, idx) => { obj[h] = rowVals[idx] ?? null; });
      out.push(obj);
    }
    return out;
  }
