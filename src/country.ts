...
try {
  ...
  countries.registerLocale((en as any).default ?? en);
 ...
 */
function tryIso2Direct(raw: string): string | null {
 ...
 */
function fuzzyMatchCountry(key: string): string | null {
  const allNames = countries.getNames("en") as Record<string, string>;
  let bestCode: string | null = null;
  let bestScore = 0;
  for (const [code, name] of Object.entries(allNames)) {
    const normName = normalizeKey(name);
    if (!normName) continue;
    if (normName === key) return code;
  ...
  if (bestCode && key.length >= 5) return bestCode;
  return null;
}
 ...
 * Signed: EyosiyasJ
 */
export function normalizeCountryToIso2(raw: string | null | undefined): string | null {
 ...
 * Signed: EyosiyasJ
 */
export function iso2ToCountryName(code: string | null | undefined): string | null {
  if (!code) return null;
